...
export default function generate(
  ...
  model: Model
    ...
    model.pathset.forEach(pathset => {
      const path = `${pathset.generate('remove', '[id%i]')}.tsx`;
      ...
      source.addFunction({
        ...
        statements: formatCode(`
          ...
          const { handlers, processing } = useFlag<${model.nameTitle}Extended>({ 
            method: 'delete',
            path:  \`/api/${pathset.generate('remove', '${router.query?.id%i}')}\`
          });
          ...
          const action = () => {
            handlers.action().then(response => {
              ...
              flash('success', _('${model.singular} removed successfully'));
                    ...
                    <div>
                      ...
                      <span>
                        ...
                        <em className="text-italic">
                          <Translate>Thats a really long time.</Translate>
                        </em>
                      </span>
                    </div>
                    <div className="mt-4">
